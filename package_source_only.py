#!/usr/bin/env python
"""
AutoQliq Source Code Packaging Script

This script packages only the essential source code files (excluding dependencies, tests,
and documentation) into a single text file with specific START/END markers for each file.
The output is compatible with the apply_packaged_codebase_enhanced.py script.

The script will:
1. Walk through the src directory only
2. Filter out dependencies and non-essential files
3. Validate file paths for compatibility
4. Generate a text file with START/END markers for each file

Usage:
    python package_source_only.py [options]
"""
...
def find_source_files(
    ...
    """
    Find all source code files, excluding dependencies and non-essential files.
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Package source code files into a single text file with START/END markers.'
    )
    parser.add_argument(
        '--root-dir', 
        default='.', 
    ...
    parser.add_argument(
        '--output', 
        default='autoqliq_source_code.txt', 
        help='Output file path (default: autoqliq_source_code.txt)'
    )
    parser.add_argument(
        '--exclude-dirs',
        nargs='+',
