"""Workflow list component for AutoQliq UI."""
...
class WorkflowList(UIComponent):
    """
    A component for displaying and selecting workflows.
    
    This component provides a listbox with a scrollbar for displaying workflows,
    along with buttons for common operations like creating, editing, and deleting workflows.
    
    Attributes:
        frame: The main frame containing all widgets
        listbox: The listbox for displaying workflows
        scrollbar: The scrollbar for the listbox
        button_frame: The frame containing the buttons
        create_button: Button for creating a new workflow
        edit_button: Button for editing the selected workflow
        delete_button: Button for deleting the selected workflow
    """
    ...
    def __init__(
        ...
        """
        Initialize a WorkflowList component.
        ...
        try:
            ...
            # Create a label
            label = ttk.Label(self.frame, text="Workflows:")
            ...
            self.logger.debug("WorkflowList component initialized")
    ...
    def set_workflows(self, workflows: List[str]) -> None:
        ...
        Raises:
            UIError: If the workflows cannot be set
        """
        try:
            # Clear the listbox
            ...
            for workflow in workflows:
                self.listbox.insert(tk.END, workflow)
            
            # Disable the edit and delete buttons
            self.edit_button.config(state=tk.DISABLED)
            self.delete_button.config(state=tk.DISABLED)
            
            self.logger.debug(f"Set {len(workflows)} workflows in the list")
        except Exception as e:
            error_msg = "Failed to set workflows in WorkflowList"
            self.logger.exception(error_msg)
            raise UIError(error_msg, component_name="WorkflowList", cause=e) from e
    ...
    def get_selected_workflow(self) -> Optional[str]:
        ...
        try:
            ...
            self.logger.error(f"Failed to get selected workflow: {e}")
            return None
    
    def select_workflow(self, workflow_name: str) -> bool:
        """
        ...
        Returns:
            True if the workflow was found and selected, False otherwise
        """
        try:
            # Find the workflow in the list
            for i in range(self.listbox.size()):
                if self.listbox.get(i) == workflow_name:
                    # Select the workflow
                    ...
                    if self._on_select_callback:
                        self._on_select_callback(workflow_name)
                    
                    return True
            
            return False
        except Exception as e:
            self.logger.error(f"Failed to select workflow '{workflow_name}': {e}")
            return False
    ...
    def _on_selection_changed(self, event: tk.Event) -> None:
        ...
        try:
            # Get the selected workflow
            selected_workflow = self.get_selected_workflow()
            ...
            # Call the selection callback
            if selected_workflow and self._on_select_callback:
                self._on_select_callback(selected_workflow)
        except Exception as e:
    ...
    def _on_edit(self) -> None:
        ...
        try:
            selected_workflow = self.get_selected_workflow()
            if selected_workflow and self._on_edit_callback:
                self._on_edit_callback(selected_workflow)
        except Exception as e:
    ...
    def _on_delete(self) -> None:
        ...
        try:
            selected_workflow = self.get_selected_workflow()
            if selected_workflow and self._on_delete_callback:
                self._on_delete_callback(selected_workflow)
        except Exception as e:
